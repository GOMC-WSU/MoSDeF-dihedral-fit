...
class TestFileReading(BaseTest):
    def test_get_from_mol2(self):
        ...
        atom_namesList, element_namesList = get_atom_names_and_elements_from_mol2(full_fn)

        assert atom_namesList == ['C1', 'C2', 'C3', 'O1', 'O2', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6']
        assert element_namesList == ['C', 'C', 'C', 'O', 'O', 'H', 'H', 'H', 'H', 'H', 'H']

    def test_get_from_pdb(self):
        fn = "gaussian_style_output_files/CT_CT_C_OH/input/starting_coords/CT_CT_C_3_OH.pdb"
        full_fn = get_mosdef_dihedral_fit_fn(fn)
        atom_namesList, element_namesList = get_atom_names_and_elements_from_pdb(full_fn)

        assert atom_namesList == ['C1', 'C2', 'C3', 'O1', 'O2', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6']
        assert element_namesList == ['C', 'C', 'C', 'O', 'O', 'H', 'H', 'H', 'H', 'H', 'H']

    def test_write_xyz_file_from_gaussian(self):
        atom_namesList = ['C1', 'C2', 'C3', 'O1', 'O2', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6']
        ...
        assert os.listdir() == ['dihedral_coords_position_1.xyz', 'dihedral_coords_position_2.xyz']
