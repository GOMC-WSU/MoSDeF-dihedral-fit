...
def get_atom_names_and_elements_from_mol2(mol2_directory_and_filename):
    ...
    with open(mol2_directory_and_filename, "r") as fp:
        ...
        for m, line_m in enumerate(readlines_mol2_file):
            ...
            if len(split_line_m) > 0 and str(split_line_m[0]) in ["@<TRIPOS>ATOM"]:
...
def write_xyz_file_from_gaussian_coordinates(atom_names_list,
    ...
    for i_iter in range(1, total_qm_scans + 1):
        ...
        comment_1_space_in_front = f" generated by Python script from Gaussian data"
...
def check_guassian_optimized_coordinate_file_correct(gaussian_optimized_coordinate_path_and_name):
        ...
        for m, line_m in enumerate(gaussian_dihedral_header_file):
            ...
            if m == 0 and len(split_line_m) == 8:
                if split_line_m[0] == 'Row' \
                    ...
                    gaussian_optimized_coordinate_header_line_correct_bool = True
    ...
    for log_file_iter, entries_to_remove_list_iter in log_files_and_entries_to_remove_dict.items():
        ...
        with open(log_file_iter, "r") as fp:
            ...
            for m, line_m in enumerate(gaussian_log_file):
                ...
                if len(split_line_m) >= 4 and \
                        split_line_m[0] == "Charge" \
                        and split_line_m[1] == "=" \
                    ...
                    all_molecule_charge_list = mdf_math.check_previous_qm_values_match(
                        all_molecule_charge_list,
                        molecule_charge,
                        'molecule charge',
                        'Gaussian',
                    ...
                    all_molecule_multiplicity_list = mdf_math.check_previous_qm_values_match(
                        all_molecule_multiplicity_list,
        ...
        if number_of_optimized_scans != len(optimized_coordinates_ang_list) \
                or number_of_optimized_scans != len(optimized_energy_hartree_list) \
                or number_of_optimized_scans != len(optimized_dihedral_angle_degrees_list):
            raise ValueError(
        ...
        if isinstance(entries_to_remove_list_iter, list) and len(entries_to_remove_list_iter) != 0:
...
def write_qm_data_files(
    ...
    if os.path.isdir(guassian_directory_name):
        os.rmdir(guassian_directory_name)
    ...
    for qm_j in range(0, len(all_dihedral_angle_degrees_list)):
        ...
        output_file_dihedral_coordinates = open(
            f"extracted_guassian_data/dihedral_coords_position_{int(qm_j+1)}.txt", "w"
        )
...
def change_gomc_ff_file_dihedral_values(
    ...
    if not isinstance(fit_dihedral_atom_types, list) \
    ...
    if isinstance(zeroed_dihedral_atom_types, list):
        if len(zeroed_dihedral_atom_types) == 4 \
        ...
        elif len(zeroed_dihedral_atom_types) >= 1 and isinstance(zeroed_dihedral_atom_types, list):
